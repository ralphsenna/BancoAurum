...
CREATE TABLE Usuario(
    ...
    usu_tipo VARCHAR(15) NOT NULL CHECK (usu_tipo IN ('Gerente', 'Cliente', 'Funcionario', 'Administrador')),
...
DELIMITER //
CREATE TRIGGER Valida_Gerente 
BEFORE INSERT ON Usuario
FOR EACH ROW
BEGIN
    IF NEW.usu_tipo = 'Gerente' AND 
    EXISTS (SELECT 1 FROM Usuario WHERE ag_codigo = NEW.ag_codigo AND usu_tipo = 'Gerente') THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Uma agência só pode ter um usuário gerente';
    END IF;
END;
// DELIMITER ;
...
CREATE TABLE Produto(
    ...
    pro_tipo VARCHAR(15) NOT NULL,
    pro_nome VARCHAR(50) NOT NULL,
    pro_saldo DECIMAL(10,2),
    pro_limite DECIMAL(10,2),
    pro_valor DECIMAL(10,2),
    pro_juros DECIMAL(10,2)
); 
...
DELIMITER //
CREATE TRIGGER Valida_Tipo_Produto
BEFORE INSERT ON Produto
FOR EACH ROW
BEGIN
    IF NEW.pro_tipo = 'Cartao Débito' THEN
        IF NEW.pro_nome IS NULL THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cartao Débito requer pro_nome';
        END IF; 
    ELSEIF NEW.pro_tipo = 'Cartao Crédito' THEN
        IF NEW.pro_saldo IS NULL OR NEW.pro_limite IS NULL THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cartao Crédito requer pro_saldo e pro_limite';
        END IF;
    ELSEIF NEW.pro_tipo = 'Seguro' THEN
        ...
        END IF;
    ELSEIF NEW.pro_tipo IN ('Emprestimo', 'Financiamento') THEN
        IF NEW.pro_valor IS NULL OR NEW.pro_juros IS NULL THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Emprestimo ou Financiamento requerem pro_valor e pro_juros';
        END IF;
    ELSE
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Tipo de produto inválido. Valores aceitos: Cartao Crédito, Cartao Débito, Empréstimo, Financiamento ou Seguro';
    END IF;
END;
// DELIMITER ;
